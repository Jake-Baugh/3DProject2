...
namespace Resources
{
	Material::Definition::Definition(ID3D10Device* device)
	...
	Material::Definition::~Definition() throw()
	{
		SafeDelete(MainTexture);
	}

	Material::Material(ID3D10Device* device)
		: mDevice(device)
	{
		Materials["default"] = new Definition(mDevice);
	}

	Material::Material(ID3D10Device* device, const std::string& filename)
		: mDevice(device)
	{
		std::string path = "Resources/Materials/" + filename;
		...
		if(!file.is_open())
			throw r2ExceptionIOM("Failed to load material: " + path);
		...
		while(!file.eof())
		{
			...
			if(key == "newmtl")
			{
				...
				currMaterial = new Definition(mDevice);
			}
			...
			else if(key == "map_Ka" || key == "map_Kd" || key == "map_Ks")
			{
				...
				if(textureFilename.find('.') != std::string::npos)
				{
					try 
					{
						currMaterial->MainTexture = new Texture(mDevice, textureFilename);
					} 
					catch (r2::Exception::IO& e)
					{
						// Failed to load texture, simply load whitePixel.png and print a warning
						currMaterial->MainTexture = new Texture(mDevice, "whitePixel.png");
